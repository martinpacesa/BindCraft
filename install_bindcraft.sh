...
if [ -n "$cuda" ]; then
  CONDA_OVERRIDE_CUDA="$cuda" $pkg_manager install \
    pip pandas matplotlib 'numpy<2.0.0' biotite scipy pdbfixer seaborn libgfortran5 tqdm jupyter ffmpeg pyrosetta fsspec py3dmol \
...
required_packages=(pip pandas libgfortran5 matplotlib numpy biotite scipy pdbfixer seaborn tqdm jupyter ffmpeg pyrosetta fsspec py3dmol chex dm-haiku dm-tree joblib ml-collections immutabledict optax jaxlib jax cuda-nvcc cudnn)
missing_packages=()
